...
sub gener_cmd{
    ...
    my $cmd = "R --vanilla --slave --input $output --output1 $fig1 --output2 $fig2  --cluster_cols $opts_sub->{cluster_cols} --cluster_rows $opts_sub->{cluster_rows}  --height $opts_sub->{height} --width $opts_sub->{width} --height2 $opts_sub->{height2} --width2 $opts_sub->{width2} < $FindBin::Bin/lib/Meth/Heatmap.R";
     print OUT "$cmd\n";
     my $r_rep = `$cmd`;
     print "$class: $r_rep\n";
}
