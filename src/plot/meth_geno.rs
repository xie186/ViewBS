...
use crate::commands::meth_geno::MethGenoTable;
...
#[cfg(feature = "plots")]
fn write_plot_with_kuva(table: &MethGenoTable, options: &PlotOptions) -> Result<OutputFile> {
    use kuva::prelude::*;

    super::validate_plot_options(options)?;
    let (plots, layout) = build_geno_plot(table, options);
    let path = table
        .path
    ...
    if options.keep_svg && !matches!(options.format, PlotFormat::Svg) {
        ...
        let (svg_plots, svg_layout) = build_geno_plot(table, options);
...
fn build_geno_plot(
    ...
    for row in &table.rows {
        by_series
            .entry(format!("{}-{}", row.sample, row.chrom))
            .or_default()
            .push((row.start as f64, row.level));
    }

    let plots = by_series
        .into_iter()
        .enumerate()
        .map(|(index, (label, mut points))| {
    ...
    let layout = Layout::auto_from_plots(&plots)
        .with_title(format!("MethGeno {}", table.context.as_str()))
        .with_x_label("Window start")
