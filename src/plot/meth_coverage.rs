...
use crate::commands::meth_coverage::CoverageTable;
...
#[cfg(feature = "plots")]
fn write_plot_with_kuva(table: &CoverageTable, options: &PlotOptions) -> Result<OutputFile> {
    use kuva::prelude::*;

    super::validate_plot_options(options)?;
    let (plots, layout) = build_coverage_plot(table, options);
    let path = table
        .path
    ...
    if options.keep_svg && !matches!(options.format, PlotFormat::Svg) {
        ...
        let (svg_plots, svg_layout) = build_coverage_plot(table, options);
...
fn build_coverage_plot(
    ...
    for row in &table.rows {
        series
            .entry(format!("{}-{}", row.sample, row.context.as_str()))
            .or_default()
            .push((row.depth as f64, row.percentage));
    }
    ...
    let layout = Layout::auto_from_plots(&plots)
        .with_title("Methylation coverage")
        .with_x_label("Depth")
