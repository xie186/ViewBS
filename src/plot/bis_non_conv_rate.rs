...
use crate::commands::bis_non_conv_rate::BisNonConvRateTable;
use crate::errors::{Result, ViewBsError};

pub(crate) fn write_plot(
    table: &BisNonConvRateTable,
    options: &PlotOptions,
...
#[cfg(feature = "plots")]
fn write_plot_with_kuva(table: &BisNonConvRateTable, options: &PlotOptions) -> Result<OutputFile> {
    use kuva::prelude::*;

    super::validate_plot_options(options)?;
    let (plots, layout) = build_bis_plot(table, options);
    let path = table
        .path
    ...
    if options.keep_svg && !matches!(options.format, PlotFormat::Svg) {
        ...
        let (svg_plots, svg_layout) = build_bis_plot(table, options);
...
fn build_bis_plot(
    ...
    let layout = Layout::auto_from_plots(&plots)
        .with_title("Bisulfite non-conversion rate")
        .with_x_label("Context")
        .with_y_label("Non-conversion rate")
