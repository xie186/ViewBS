...
use crate::commands::meth_one_region::MethOneRegionTable;
use crate::errors::{Result, ViewBsError};

pub(crate) fn write_plot(
    table: &MethOneRegionTable,
    options: &PlotOptions,
...
#[cfg(feature = "plots")]
fn write_plot_with_kuva(table: &MethOneRegionTable, options: &PlotOptions) -> Result<OutputFile> {
    use kuva::prelude::*;

    super::validate_plot_options(options)?;
    let (plots, layout) = build_one_region_plot(table, options);
    let path = table
        .path
    ...
    if options.keep_svg && !matches!(options.format, PlotFormat::Svg) {
        ...
        let (svg_plots, svg_layout) = build_one_region_plot(table, options);
...
fn build_one_region_plot(
    ...
    for row in &table.rows {
        by_sample
            ...
            .or_default()
            .push((row.position as f64, row.level));
    }
    ...
    let layout = Layout::auto_from_plots(&plots)
        .with_title(format!("MethOneRegion {}", table.context.as_str()))
